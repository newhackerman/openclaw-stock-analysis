...
### Phase 1: 数据获取（带重试机制）⭐ NEW
1. 识别股票代码和名称
2. 获取实时行情（每个数据源最大重试 **3 次**）
3. 获取财务数据（每个数据源最大重试 **3 次**）
4. 获取 K 线数据（多源备份，每源重试 **3 次**）
5. 获取同业公司数据
...
|------|------|
| analyze.sh | 综合分析主入口 |
| fetch-eastmoney.sh | 东方财富数据（带重试） |
| fetch-ths.sh | 同花顺数据（带重试） |
| fetch-price.sh | 实时价格（多数据源 + 重试） |
| fetch-kline.sh | K 线数据（多源 + 重试） |
| technical-analysis.sh | 技术分析（带重试） |
| calc-indicators.sh | 技术指标计算 |
| calc-valuation.sh | 估值计算 |
| parse-financials.sh | 数据解析 |
| fetch-research.sh | 券商研报（带重试） |
| compare-stocks.sh | 同业对比 🆕 |
| lib-retry.sh | 🔧 通用重试函数库 |
...
### lib-retry.sh 函数库提供的工具：

#### `curl_retry` - HTTP 请求带重试
```bash
curl_retry "URL" [max_retries] [timeout]
# 示例：curl_retry "https://api.example.com/data" 3 30
```
- 参数 1：URL 地址
- 参数 2：最大重试次数（默认 3）
- 参数 3：超时时间秒数（默认 30）
...
#### `multi_source_fetch` - 多源数据获取
```bash
multi_source_fetch "url1" "url2" "url3"
# 依次尝试多个数据源，每源重试 3 次
```
- 支持定义多个备用 URL
- 成功获取第一个有效数据即返回
...
### v1.4.0 - 增加智能重试机制 ⭐ NEW
- ✨ 新增 `lib-retry.sh` 通用重试函数库
- ✨ 所有数据获取脚本均支持 3 次自动重试
- ✨ 失败后自动切换到备用数据源
- ✨ 友好的进度提示和错误信息
- 🔧 优化了 fetch-price.sh、fetch-eastmoney.sh 等脚本
...
./fetch-price.sh <股票代码>           # 自动重试 3 次
./fetch-eastmoney.sh <股票代码> financial  # 自动重试 3 次
./technical-analysis.sh <股票代码>    # 多源自动重试
```
...
```bash
# 基本用法
./analyze.sh 1.600879 航天电子

# 获取单一数据类型
./fetch-price.sh 600879              # 股价，自动重试
./fetch-eastmoney.sh 1.600879 price  # 行情数据
./fetch-kline.sh 600879 day          # 日线数据
./technical-analysis.sh 600879       # 技术分析
```
